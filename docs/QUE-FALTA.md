...
| 2.21 | 🔴 **La ficha publicada decía «English» y la causa NO estaba en App Store Connect** | La fila **Languages** del App Store sale del **binario**, no del *Primary Language* de la ficha: Apple la infiere de las carpetas `.lproj` del bundle y, si no hay ninguna, de `CFBundleLocalizations`. Este proyecto **no tenía ni una cosa ni la otra** —`CFBundleDevelopmentRegion` era `$(DEVELOPMENT_LANGUAGE)`, o sea `en`— así que el binario declaraba inglés mientras la ficha estaba en español. Arreglado el 2026-09-10 en `app.json` → `ios.infoPlist`: `CFBundleDevelopmentRegion: "es"` y `CFBundleLocalizations: ["es"]`. ⚠️ **Viaja en el binario: no cambia hasta que Apple apruebe un build nuevo**, y editar `ios/Info.plist` a mano no sirve porque `/ios` está en `.gitignore` y EAS hace su propio `prebuild` desde `app.json` |
...
| 3.1.2 | ✅ **Verificado el 2026-09-03**: hay **1 fila** en `push_tokens` con `platform = 'android'`, vista por última vez a las 15:54 UTC — la más reciente de toda la tabla. Sumado al recorrido de sismos sembrados del 2026-09-02, donde el Android despertó en segundo plano y capturó ubicación a los 4,6 s, **el push de Android funciona de punta a punta en el dev client**. Lo que eso NO prueba es la app firmada por Play App Signing, que usa credenciales distintas: se vuelve a mirar tras la primera subida al track interno (3.2.5) |
...
| 3.3.4 | 🔴 **Sigue pendiente, y es lo único que separa «el paywall se ve» de «el cobro funciona».** Comprobado el 2026-09-03: `revenuecat_events` tiene **cero** eventos con `store = PLAY_STORE`. Los 17 que hay son todos `APP_STORE`, en sandbox. Que el SDK liste los productos en Android no prueba nada del otro extremo: falta una compra que atraviese **Play → RevenueCat → webhook → `is_premium = true`**. Si esa cadena está cortada, el usuario paga y no recibe nada, sin ningún error visible. La consulta que lo cierra: `select payload->'event'->>'store', type, received_at from public.revenuecat_events order by received_at desc limit 5;` — tiene que aparecer `PLAY_STORE`. Probar una compra en el track de prueba. ⚠️ **No se puede desde un APK instalado a mano.** Play Billing solo responde si el build llegó por Play: hay que subirlo a un track de **internal testing** (3.2.5) y comprar con una cuenta declarada **license tester** en Play Console. Con un APK de depuración sale «el artículo que solicitaste no está disponible», que parece un fallo de la app y no lo es |
