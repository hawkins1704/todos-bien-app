...
| `0025_circle_alert_scope` | `get_circle` devuelve `alerted_quake_ids`: a qué contactos les llegó cada sismo de las últimas 6 h. Sin ese dato la Home marcaba «sin confirmar» a quien nunca fue alertado, contradiciendo al propio servidor, que desde 0020 ya los excluye de «X no responde». El dato sale de una función `security definer` **sin parámetros** —el usuario lo pone `auth.uid()`—, así que `alert_deliveries` sigue con RLS cerrado a cal y canto |
...
- ~~**Notificaciones del feed Global.** …pero **nadie encola** esos avisos: el fan-out solo
  cubre la alerta de sismo cercano, no el feed mundial.~~
  ✅ **Falso desde la 0021, y este pendiente quedó vivo por error. Comprobado el 2026-09-08.**
  `private.fan_out_quake` llama a `private.notify_quake_news(q.id)` en la misma pasada, y esa
  función encola **dos** tipos: `quake_national` (M ≥ 4,5, tu país, gratis) y `quake_worldwide`
  (M ≥ 6,0, país distinto, **solo `is_premium`**). En producción hay **5 `quake_worldwide` ya
  entregadas** y 27 `quake_national`. No hay nada que construir.
...
| Fecha | Qué pasó |
|---|---|
| 2026-09-08 | 🔍 **Auditoría del corte de notificaciones, leyendo los cuerpos de las funciones vivas en vez de los documentos.** La pregunta era simple —«¿qué notificaciones agrega Premium exactamente?»— y encontró tres cosas. **(1)** La tabla 2×2 de `MONETIZACION.md` §«el corte real» **prometía «aviso al minuto 0»**, que la migración **0030 eliminó** cuatro días después de que esa tabla se escribiera. Se contradecía con §3.3 del mismo archivo. Corregida, con el inventario exacto al lado: Premium agrega **tres** cosas y ninguna es un tipo de aviso nuevo — «está bien» y «no responde» **cruzando la zona del sismo**, más la noticia mundial M≥6,0. `QUE-PROMETE-LA-APP.md` §7 nunca se equivocó; el error vivió solo en Monetización. **(2)** El pendiente «nadie encola los avisos del feed Global» es **falso desde la 0021**: `fan_out_quake` llama a `notify_quake_news`, y hay 5 `quake_worldwide` entregadas. **(3)** `quake_applies` recibe `p_is_premium` y `p_worldwide_enabled` y **no usa ninguno** — deuda 1.17. **Y un error propio que vale más que los tres:** se reportó que `contact_is_safe` «nunca se envió, cero veces», leyendo `notification_deliveries`. Falso: se verificó en dos teléfonos el 2026-09-01 (7b.4) y **la evidencia la borramos nosotros** el 2026-09-06 al limpiar los sismos sembrados —que se llamaban `verif-guardian-0901-a/b`—. `alert_deliveries` quedó con 3 filas de agosto. **Contar filas de una tabla que se limpia a propósito no dice si una función anda**, y el contero miente siempre hacia abajo. Anotado en `VERIFICACION-EN-DISPOSITIVO.md` §7.b para quien vuelva a mirar. |
| 2026-09-08 | 🔄 **Cambio de tesis comercial: el comprador dejó de ser la diáspora y pasó a ser el peruano que vive en el Perú.** `MONETIZACION.md` estaba construido sobre «son las 3 AM en Madrid», y al reenfocar al mercado peruano hubo que revisar §2, §3.1, §6 y descartar el pendiente de fichas distintas para tiendas del exterior. **El escenario que sostiene el precio es ahora Lima↔provincia**: un M6,8 en Arequipa no alcanza a Lima igual que no alcanza a Madrid, así que Guardián sigue siendo el único canal para el sismo que no te tocó — mismo mecanismo, mercado mucho más grande y sin depender de que alguien en Miami encuentre la app. **No se tocó una línea de código:** precios, corte gratis/Premium y migraciones quedan igual. También se alineó la landing (el hero de Premium decía Madrid) y se decidió qué hacer con las **mochilas de emergencia**: `MONETIZACION.md` §7 nuevo, con la cuenta que lo define —el pack cuesta S/57-65 puesto en la puerta y una venta vitalicia deja S/57,55 neto, así que regalarlo por suscriptor es margen cero—. **Las entradas de esta bitácora anteriores a hoy hablan de Madrid y se dejan como están**: eran ciertas cuando se escribieron, y el registro del cambio vive en `MONETIZACION.md` §2 y §6. |
